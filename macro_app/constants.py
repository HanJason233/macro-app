...
ACTION_RESIZE_WINDOW: Final[str] = "窗口大小"
...
LEGACY_ACTION_ALIASES: Final[dict[str, str]] = {
    ...
    "缁濆鍧愭爣鐐瑰嚮": ACTION_CLICK_ABS,
    "绐楀彛鐩稿鐐瑰嚮": ACTION_CLICK_REL,
...
ACTION_DEFINITIONS: Final[ActionDefMap] = {
    ...
    ACTION_CLICK_ABS: [("x", "绝对X", "500"), ("y", "绝对Y", "500"), ("button", "按键", "left"), ("clicks", "点击次数", "1")],
    ACTION_CLICK_REL: [("x", "相对X", "100"), ("y", "相对Y", "100"), ("button", "按键", "left"), ("clicks", "点击次数", "1")],
    ACTION_CLICK_IMAGE: [
        ("image_path", "模板图片", ""),
...
ACTION_GROUPS: Final[dict[str, list[str]]] = {
    ...
    "鼠标点击": [ACTION_CLICK_ABS, ACTION_CLICK_REL, ACTION_CLICK_IMAGE, ACTION_CLICK_OCR],
    "检测": [ACTION_DETECT_IMAGE, ACTION_DETECT_OCR, ACTION_DETECT_WINDOW_SIZE, ACTION_GET_WINDOW_SIZE],
...
def summarize_step(step: dict) -> str:
    ...
    if action == ACTION_CLICK_ABS:
        return f"绝对点击 ({params.get('x')}, {params.get('y')}){suffix}"
    if action == ACTION_CLICK_REL:
        return f"窗口点击 ({params.get('x')}, {params.get('y')}){suffix}"
    ...
    if action == ACTION_RESIZE_WINDOW:
        return f"窗口大小 {params.get('width')} x {params.get('height')}{suffix}"
